...
This branch fixes the critical foundation issues and keeps unsupported generated sections out of active templates. Connected-store review also found policy/configuration conflicts that cannot safely be solved in theme code: the shipping policy says U.S.-only shipping while Shopify reports a much broader country set, the Terms of Service page contains literal template placeholders, the contact policy repeats its contact block, and the store description is blank. These require merchant decisions or production-content changes.
...
| Shipping and returns | Shipping policy confirms free standard U.S. shipping at $100+, 5–7 business-day transit, 1–2 business-day processing, and no international shipping. Shopify's enabled destination data reports a much broader country set. The return policy confirms a 30-day window and eligibility rules but does not promise free returns. | Customers can receive contradictory availability or cost messages; “free returns” wording is unsupported. | Critical | Reconcile Markets/shipping zones with the published policy, confirm exclusions, and remove or revise any “free returns” promise. | Shopify Markets/shipping settings; policy pages; header/footer group JSON; `config/settings_data.json` | **Merchant input / Shopify admin** |
...
| Terms of Service content | The rendered Terms page contains literal `[LINK]` placeholders and a merchant drafting note. | Public legal content appears unfinished and can reduce trust or create legal ambiguity. | Critical | Have qualified merchant/legal review replace all placeholders and remove drafting notes in Shopify policy settings. | Shopify policy content | **Merchant input / Shopify admin** |
| Contact policy content | The rendered contact-information policy repeats the business/contact block. | Looks unpolished and makes legal/support information harder to scan. | Medium | Consolidate to one approved contact block in Shopify policy settings. | Shopify policy content | **Merchant input / Shopify admin** |
...
- Post-sync theme-editor schema rendering and section-setting migration.
- Lighthouse/Core Web Vitals and app-by-app performance profiling.
...
1. Upload and approve the four homepage editorial images and their alt text; the matching original filenames were not present in Shopify Files.
...
7. Correct the Terms and contact policy content, and reconcile international shipping configuration with the published U.S.-only policy.
